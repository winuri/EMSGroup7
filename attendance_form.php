...
$conn = new mysqli('localhost', 'root', '', 'emsdatabase_new');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($_POST as $key => $value) {
        if (strpos($key, 'arrival_time_') === 0) {
            ...
            // Insert attendance record into database only if status is 'Present'
            if ($status === 'Present') {
                $insert_query = "INSERT INTO attendance (EMP_ID, attendance_date, arrival_time, leave_time, status) 
                                 VALUES ('$emp_id', '$date', '$arrival_time', '$leave_time', '$status')";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td>{$row['F_name']} {$row['L_name']}</td>
                                    <td>{$row['Work_name']}</td>
                                    <td><input type='time' name='arrival_time_{$row['EMP_ID']}' value='08:00' required></td>
                                    <td>
                                        <select name='leave_time_{$row['EMP_ID']}' required>
